        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('/company/role0/jobOrderWorker/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
                                ...
                                <td>
                                    <div>
                                        ...
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="info('工作包接单详情-{{$data->id}}','{{URL::asset('company/role0/jobOrderWorker/info')}}?id={{$data->id}}',{{$data->id}})">
                                            详情
                                        </button>
                                    </div>
                                </td>
    ...
    <script type="text/javascript">
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role0/jobOrderWorker/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/company/role0/jobOrderWorker/index')}}');--}}
            reloadPage();
        }
        ...
        function resetPassword(obj, id) {
            ...
            layer.confirm('确认要重置密码？', function (index) {
                ...
                ajaxRequest('{{URL::asset('')}}' + "company/role0/jobOrderWorker/resetPassword/" + id, param, "GET", function (ret) {
                    if (ret.result == true) {
                        $("#search_form").submit();
                        $(".btn-refresh").click();
                    }
