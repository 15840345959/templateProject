        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="get"
                      action="{{URL::asset('/company/role0/jobOrderItem/batchClockIn')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
    ...
    <script type="text/javascript">
        ...
        layui.use(['index', 'layer', 'form', 'set', 'laypage'], function () {
            ...
            form.on('submit(LAY-form-edit)', function (data) {
                ...
                layer.confirm('确认要进行批量打卡吗？', function (index) {
                    var param = data.field;
                    consoledebug.log("param:" + JSON.stringify(param));
                    var loadding_index = layer.load(1, {time: 10 * 1000});   //加载

                    //进行请求
                    ajaxRequest('{{URL::asset('/company/role0/jobOrderItem/batchClockIn')}}', param, "POST", function (ret) {
                        if (ret.result) {
                            layer.msg('批量打卡成功', {icon: 1, time: 1000});
                            $("#search_form").submit();
                        } else {
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role0/jobOrderItem/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/company/role0/jobOrderItem/batchClockIn')}}');--}}
            reloadPage();
        }
        ...
        function resetPassword(obj, id) {
            ...
            layer.confirm('确认要重置密码？', function (index) {
                ...
                ajaxRequest('{{URL::asset('')}}' + "company/role0/jobOrderItem/resetPassword/" + id, param, "GET", function (ret) {
                    if (ret.result == true) {
                        $("#search_form").submit();
                        $(".btn-refresh").click();
                    }
