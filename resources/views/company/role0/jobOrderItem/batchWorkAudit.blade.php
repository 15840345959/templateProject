        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="get"
                      action="{{URL::asset('/company/role0/jobOrderItem/batchWorkAudit')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
...
@section('script')
    <script type="text/javascript">
        ...
        layui.use(['index', 'layer', 'form', 'set', 'laypage'], function () {
            ...
            form.on('submit(LAY-form-edit)', function (data) {
                layer.confirm('确认要进行批量考核吗？考核默认评分为3分，工资、工时将按照计划工资、工时设置', function (index) {
                    ...
                    //进行请求
                    ajaxRequest('{{URL::asset('/company/role0/jobOrderItem/batchWorkAudit')}}', param, "POST", function (ret) {
                        if (ret.result) {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/company/role0/jobOrderItem/batchWorkAudit')}}');--}}
            reloadPage();
        }
