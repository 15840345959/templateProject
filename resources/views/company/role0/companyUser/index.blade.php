        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('/company/role0/companyUser/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
            ...
            <div class="layui-card-body">
                <div style="padding-bottom: 10px;">
                    <button class="layui-btn layuiadmin-btn-forum-list"
                            onclick="edit('添加人员','{{URL::asset('company/role0/companyUser/edit')}}')">
                        添加人员
                    </button>
                </div>
                <div class="table-container">
                    <table class="layui-table text-c" style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <th class="text-c" width="20">名称</th>
                            <th class="text-c" width="20">手机号</th>
                            <th class="text-c" width="60">所属公司/项目</th>
                            <th class="text-c" width="20">角色</th>
                        ...
                        @foreach($datas as $data)
                            <tr>
                                ...
                                <td>
                                    <div class="text-oneline">
                                        @if(isset($data->man_company))
                                            {{$data->man_company->name}}
                                        @endif
                                        @if(isset($data->bus_company))
                                ...
                                <td>
                                    <div>
                                        ...
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="edit('编辑人员-{{$data->name}}','{{URL::asset('/company/role0/companyUser/edit')}}?id={{$data->id}}&man_company_id={{$data->man_company_id}}',{{$data->id}})">
                                            编辑
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="resetPassword(this,'{{$data->id}}')">
                                            重置
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="show_ewm('职员工卡','{{URL::asset('/company/role0/companyUser/ewm')}}?id={{$data->id}}',{{$data->id}})">
                                            工卡
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="setUser('职员绑定-{{$data->name}}','{{URL::asset('/company/role0/companyUser/setUser')}}?id={{$data->id}}',{{$data->id}})">
                                            绑定用户
                                        </button>
                                    </div>
                                </td>
    ...
    <script type="text/javascript">
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role0/companyUser/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/company/role0/companyUser/index')}}');--}}
            reloadPage();
        }
        ...
        function resetPassword(obj, id) {
            ...
            layer.confirm('确认要重置密码？', function (index) {
                ...
                ajaxRequest('{{URL::asset('')}}' + "company/role0/companyUser/resetPassword/" + id, param, "GET", function (ret) {
                    if (ret.result == true) {
                        $("#search_form").submit();
                        $(".btn-refresh").click();
                    }
