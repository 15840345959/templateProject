        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('/company/role0/busCompany/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
            ...
            <div class="layui-card-body">
                <div style="padding-bottom: 10px;">
                    <button class="layui-btn layuiadmin-btn-forum-list"
                            onclick="edit('添加项目','{{URL::asset('company/role0/busCompany/edit')}}')">
                        添加项目
                    </button>
                </div>
                <div>
                    <table class="layui-table text-c" style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <th class="text-c" width="60">所属物业</th>
                        ...
                        @foreach($datas as $data)
                            <tr>
                                ...
                                <td>
                                    <div class="text-oneline">{{$data->man_company->name}}</div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div>
                                        ...
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="edit('编辑在管项目-{{$data->name}}','{{URL::asset('/company/role0/busCompany/edit')}}?id={{$data->id}}&man_company_id={{$data->man_company_id}}',{{$data->id}})">
                                            编辑
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="edit_worker('人员管理-{{$data->name}}','{{URL::asset('/company/role0/busCompany/worker')}}?id={{$data->id}}',{{$data->id}})">
                                            人员
                                        </button>
                                    </div>
                                </td>
    ...
    <script type="text/javascript">
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role0/busCompany/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/company/role0/busCompany/index')}}');--}}
            reloadPage();
        }
         ...
         * 
         */
        function edit_worker(title, url, id) {
            console.log("edit_worker url:" + url);
