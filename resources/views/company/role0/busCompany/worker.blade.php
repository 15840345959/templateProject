...
@section('script')
    <script type="text/javascript">
        ...
        function search_worker() {
            ...
            ajaxRequest('{{URL::asset('/company/role0/worker/getListByCon')}}', param, "GET", function (ret) {
                if (ret.result) {
        ...
        function add_worker(obj, worker_id, bus_company_id) {
            layer.confirm('确认要加入吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role0/companyWorker/add')}}', param, "POST", function (ret) {
                    if (ret.result) {
                        layer.msg('已经加入');
                        refresh();
                    } else {
        ...
        function remove_worker(obj, id) {
            layer.confirm('确认要调离吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role0/companyWorker/remove')}}', param, "POST", function (ret) {
                    if (ret.result) {
                        layer.msg('已经调离');
                        refresh();
                    } else {
         ...
         */
        function refresh() {
            {{--location.replace('{{URL::asset('/company/role0/busCompany/worker')}}?id={{$data->id}}');--}}
            reloadPage();
        }
