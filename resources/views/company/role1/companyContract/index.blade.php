        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('company/role1/companyContract/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
            ...
            <div class="layui-card-body">
                <div style="padding-bottom: 10px;">
                    <button class="layui-btn layuiadmin-btn-forum-list"
                            onclick="edit('添加合同','{{URL::asset('company/role1/companyContract/edit')}}')">
                        添加合同
                    </button>
                </div>
                <div>
                            ...
                            <tr class="{{$data->alert_level}}" title="{{$data->alert_str}}">
                                ...
                                <td>
                                    <div>
                                        ...
                                        @endif
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="edit('编辑合同-{{$data->name}}','{{URL::asset('/company/role1/companyContract/edit')}}?id={{$data->id}}&man_company_id={{$data->man_company_id}}',{{$data->id}})">
                                            编辑
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
    ...
    <script type="text/javascript">
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role1/companyContract/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
