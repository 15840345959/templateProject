                            ...
                            <tr>
                                ...
                                <td colspan="4">
                                    ...
                                    @if($data->audit_status==\App\Components\Common\Utils::STATUS_VALUE_0)
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="work_audit('工作考评-{{$data->id}}','{{URL::asset('company/role1/jobOrderItem/workAudit')}}?id={{$data->id}}',{{$data->id}})">
                                            工作考评
                                        </button>
                                    @endif
    ...
    <script type="text/javascript">
        ...
        function sub_clockIn(obj, id) {
            layer.confirm('确认要代TA打卡吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role1/jobOrderItem/subClockIn')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('打卡成功');
                        $(".btn-refresh").click();
                    } else {
