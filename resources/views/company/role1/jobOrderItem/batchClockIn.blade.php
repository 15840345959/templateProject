        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="get"
                      action="{{URL::asset('/company/role1/jobOrderItem/batchClockIn')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
...
@section('script')
    <script type="text/javascript">
        ...
        layui.use(['index', 'layer', 'form', 'set', 'laypage'], function () {
            ...
            form.on('submit(LAY-form-edit)', function (data) {
                ...
                layer.confirm('确认要进行批量打卡吗？', function (index) {
                    ...
                    //进行请求
                    ajaxRequest('{{URL::asset('/company/role1/jobOrderItem/batchClockIn')}}', param, "POST", function (ret) {
                        if (ret.result) {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/company/role1/jobOrderItem/batchClockIn')}}');--}}
            reloadPage();
        }
