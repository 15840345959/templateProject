        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('/company/role1/exportWorkerSalary/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
            ...
            <div class="layui-card-body">
                <div style="padding-bottom: 10px;">
                    <button class="layui-btn layuiadmin-btn-forum-list"
                            onclick="export_salary('导出工资单','{{URL::asset('company/role1/exportWorkerSalary/export')}}')">
                        导出工资单
                    </button>
                </div>
    ...
    <script type="text/javascript">
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/company/role1/exportWorkerSalary/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/company/role1/exportWorkerSalary/index')}}');--}}
            reloadPage();
        }
        ...
        function resetPassword(obj, id) {
            ...
            layer.confirm('确认要重置密码？', function (index) {
                ...
                ajaxRequest('{{URL::asset('')}}' + "company/role1/exportWorkerSalary/resetPassword/" + id, param, "GET", function (ret) {
                    if (ret.result == true) {
                        $("#search_form").submit();
                        $(".btn-refresh").click();
                    }
