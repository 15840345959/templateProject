        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('/admin/manCompany/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
            ...
            <div class="layui-card-body">
                <div style="padding-bottom: 10px;">
                    <button class="layui-btn layuiadmin-btn-forum-list"
                            onclick="edit('添加物业公司','{{URL::asset('/admin/manCompany/edit')}}')">
                        添加物业公司
                    </button>
                </div>
                <div>
                    <table class="layui-table text-c" style="width: 100%;">
                        ...
                        @foreach($datas as $data)
                            <tr>
                                ...
                                <td>
                                    <div class="text-oneline">{{$data->address}}</div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div>
                                        ...
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="edit('编辑物业公司-{{$data->name}}','{{URL::asset('/admin/manCompany/edit')}}?id={{$data->id}}',{{$data->id}})">
                                            编辑
                                        </button>
                                    </div>
                                </td>
    ...
    <script type="text/javascript">
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/admin/manCompany/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/admin/manCompany/index')}}');--}}
            reloadPage();
        }
