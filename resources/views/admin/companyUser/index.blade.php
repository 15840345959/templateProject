        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('/admin/companyUser/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
                            ...
                            <tr>
                                ...
                                <td>
                                    <div>
                                        ...
                                        @endif
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="edit('编辑人员-{{$data->name}}','{{URL::asset('/admin/companyUser/edit')}}?id={{$data->id}}&man_company_id={{$data->man_company_id}}',{{$data->id}})">
                                            编辑
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                        ...
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="show_ewm('职员工卡','{{URL::asset('/admin/companyUser/ewm')}}?id={{$data->id}}',{{$data->id}})">
                                            工卡
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="setUser('职员绑定-{{$data->name}}','{{URL::asset('/admin/companyUser/setUser')}}?id={{$data->id}}',{{$data->id}})">
                                            绑定用户
                                        </button>
                                    </div>
                                </td>
...
@section('script')
    <script type="text/javascript">
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/admin/companyUser/index')}}');--}}
            reloadPage();
        }
        ...
        function resetPassword(obj, id) {
            ...
            layer.confirm('确认要重置密码？', function (index) {
                ...
                ajaxRequest('{{URL::asset('')}}' + "admin/companyUser/resetPassword/" + id, param, "GET", function (ret) {
                    if (ret.result == true) {
                        $("#search_form").submit();
                        $(".btn-refresh").click();
                    }
