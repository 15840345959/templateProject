        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('/admin/worker/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
            ...
            <div class="layui-card-body">
                <div style="padding-bottom: 10px;">
                    <button class="layui-btn layuiadmin-btn-forum-list"
                            onclick="edit('添加人员','{{URL::asset('admin/worker/edit')}}')">
                        添加人员
                    </button>
                    <button class="layui-btn layuiadmin-btn-forum-list"
                            onclick="import_worker('Excel表批量导入人员','{{URL::asset('admin/importWorkerTask/import')}}')">
                        Excel表批量导入人员
                    </button>
                </div>
                            ...
                            <tr class="{{$data->alert_level}}" title="{{$data->alert_str}}">
                                ...
                                <td>
                                    <div>
                                        ...
                                        @endif
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="edit('编辑人员-{{$data->name}}','{{URL::asset('admin/worker/edit')}}?id={{$data->id}}',{{$data->id}})">
                                            编辑
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="info('员工信息-{{$data->name}}','{{URL::asset('admin/worker/info')}}?id={{$data->id}}',{{$data->id}})">
                                            详情
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="show_ewm('员工工卡-{{$data->name}}','{{URL::asset('/admin/worker/ewm')}}?id={{$data->id}}',{{$data->id}})">
                                            工卡
                                        </button>
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="setUser('职员绑定-{{$data->name}}','{{URL::asset('/admin/worker/setUser')}}?id={{$data->id}}',{{$data->id}})">
                                            绑定用户
                                        </button>
                                    </div>
                                </td>
    ...
    <script type="text/javascript">
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/admin/worker/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
        ...
        function refresh() {
            $('#search_form')[0].reset();
            {{--location.replace('{{URL::asset('/admin/worker/index')}}');--}}
            reloadPage();
        }
        ...
        function reject(obj, id) {
            layer.confirm('确认要驳回吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/admin/worker/reject')}}', param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经驳回');
                        $("#search_form").submit();
                    } else {
