                            ...
                            <tr>
                                ...
                                <td colspan="4">
                                    <button class="layui-btn layui-btn-sm"
                                            onclick="set_insurance_no('设置保单号-{{$data->id}}','{{URL::asset('admin/jobOrderWorker/setInsuranceNo')}}?id={{$data->id}}',{{$data->id}})">
                                        设置保单
                                    </button>
                                    <button class="layui-btn layui-btn-sm"
                                            onclick="detail_item('任务明细-{{$data->id}}','{{URL::asset('admin/jobOrderItem/index')}}?job_order_worker_id={{$data->id}}',{{$data->id}})">
                                        任务明细
                                    </button>
    ...
    <script type="text/javascript">
        ...
        function approve(obj, id) {
            layer.confirm('确认要通过吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/admin/jobOrderWorker/approve')}}', param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('审核通过');
                        refresh();
                    } else {
        ...
        function reject(obj, id) {
            layer.confirm('确认要驳回吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/admin/jobOrderWorker/reject')}}', param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经驳回');
                        refresh();
                    } else {
