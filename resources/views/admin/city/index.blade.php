        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <form id="search_form" class="layui-form" method="post"
                      action="{{URL::asset('/admin/city/index')}}?page={{$datas->currentPage()}}">
                    {{csrf_field()}}
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label">关键字</label>
                            <div class="layui-input-block">
                                <input type="text" name="search_word" placeholder="请输入城市名" autocomplete="off"
            ...
            <div class="layui-card-body">
                <div style="padding-bottom: 10px;">
                    <button class="layui-btn layuiadmin-btn-forum-list"
                            onclick="edit('添加城市','{{URL::asset('/admin/city/edit')}}')">
                        添加城市
                    </button>
                </div>
                <div>
                    <table class="layui-table text-c" style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <th class="text-c" width="80">名称</th>
                            <th class="text-c" width="20">最低工资</th>
                            <th class="text-c" width="20">国码</th>
                            {{--<th class="text-c" width="20">腾讯</th>--}}
                            {{--<th class="text-c" width="20">高德</th>--}}
                            {{--<th class="text-c" width="20">百度</th>--}}
                            <th class="text-c" width="40">默认</th>
                            <th class="text-c" width="40">状态</th>
                            <th class="text-c" width="200">操作</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($datas as $data)
                            <tr>
                                <td>{{$data->id}}</td>
                                <td>{{$data->name}}</td>
                                <td>{{$data->min_wage}} 元/小时</td>
                                <td>{{isset($data->code)?$data->code:'--'}}</td>
                                {{--<td>{{isset($data->tx_code)?$data->tx_code:'--'}}</td>--}}
                                {{--<td>{{isset($data->gd_code)?$data->gd_code:'--'}}</td>--}}
                                {{--<td>{{isset($data->bd_code)?$data->bd_code:'--'}}</td>--}}
                                <td>
                                    {{($data->is_default=='0') ? '否' : '是' }}
                                </td>
                                <td>
                                    ...
                                    <div>
                                        ...
                                        @if($data->is_default==0)
                                            <button class="layui-btn layui-btn-sm"
                                                    onclick="setDefault(this,{{$data->id}})">
                                                默认
                                            </button>
                                        @else
                                            <button class="layui-btn layui-btn-sm"
                                                    onclick="cancelDefault(this,{{$data->id}})">
                                                普通
                                            </button>
                                        @endif
                                        <button class="layui-btn layui-btn-sm"
                                                onclick="edit('编辑城市','{{URL::asset('/admin/city/edit')}}?id={{$data->id}}',{{$data->id}})">
                                            编辑
                                        </button>
                                    </div>
                                </td>
    ...
    <script type="text/javascript">
        ...
        function start(obj, id) {
            layer.confirm('确认要启用吗？', function (index) {
                ...
                ajaxRequest('{{URL::asset('/admin/city/setStatus')}}/' + id, param, "GET", function (ret) {
                    if (ret.result) {
                        layer.msg('已经启用');
                        $("#search_form").submit();
                    } else {
        ...
        function setDefault(obj, id) {
            layer.confirm('确认要选为默认吗？', function (index) {
                //此处请求后台程序，下方是成功后的前台处理
                var param = {
                    id: id,
                    is_default: 1,
